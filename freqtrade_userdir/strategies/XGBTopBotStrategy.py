...
class XGBTopBotStrategy(IStrategy):
    ...
    def __init__(self, config: dict) -> None:
        ...
        # Model paths via env: bottom for up (buy), top for down (sell)
        self.regressor_path = os.environ.get("XGB_REG_PATH", "")
        self.up_model_path = os.environ.get("XGB_UP_PATH", "")  # Bottom model
        self.down_model_path = os.environ.get("XGB_DN_PATH", "")  # Top model
        # Thresholds and gating
        self.p_up_thr = float(os.environ.get("XGB_P_UP_THR", "0.6"))
        self.p_dn_thr = float(os.environ.get("XGB_P_DN_THR", "0.6"))
        self.p_margin = float(os.environ.get("XGB_P_MARGIN", "0.0"))
        self.min_hold_bars = int(os.environ.get("XGB_MIN_HOLD", "4"))
        ...
        # Lazy model loading
        self._reg = None
        self._upc = None  # Bottom classifier
        self._dnc = None  # Top classifier
    ...
    def _load_xgb(self):
        ...
        device = os.environ.get("XGB_DEVICE", "cpu").lower()
    ...
    def populate_indicators(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Gating: thresholds + margin; optional regressor direction
        reg_gate = True
        if mu is not None:
            reg_gate_up = mu > 0.0
            reg_gate_dn = mu < 0.0
        ...
        up_ok = np.zeros(n, dtype=bool)  # Bottom signal for long
        dn_ok = np.zeros(n, dtype=bool)  # Top signal for short
        if p_up is not None:
        ...
        # Convert to entry/exit with min-hold and cooldown
        last_change = -10**9
        cooldown_until = -1
        pos = 0
        for t in range(n):
            ...
            if up_ok[t] and not dn_ok[t]:
                desired = +1  # Long on bottom
            elif dn_ok[t] and not up_ok[t]:
                desired = -1  # Short on top
            else:
                desired = 0
